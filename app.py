...
data = {
    ...
    "Tension artérielle systolique (mmHg)": st.sidebar.number_input("Tension systolique (mmHg)", min_value=50, max_value=200, step=1),
    "Tension artérielle diastolique (mmHg)": st.sidebar.number_input("Tension diastolique (mmHg)", min_value=30, max_value=130, step=1),
    "Potassium (K+, mmol/L)": st.sidebar.number_input("Potassium (K+, mmol/L)", min_value=1.0, max_value=10.0, step=0.1),
    "Calcium (Ca++, mmol/L)": st.sidebar.number_input("Calcium (Ca++, mmol/L)", min_value=0.5, max_value=5.0, step=0.1),
    "Phosphore (P, mmol/L)": st.sidebar.number_input("Phosphore (P, mmol/L)", min_value=0.5, max_value=5.0, step=0.1),
    "Créatinine (µmol/L)": st.sidebar.number_input("Créatinine (µmol/L)", min_value=10, max_value=1000, step=1),
    "Diurèse (mL/24h)": st.sidebar.number_input("Diurèse (mL/24h)", min_value=0, max_value=5000, step=10)
}
...
if st.sidebar.button("Enregistrer les données"):
    try:
        ...
        if "surveillance_data.csv" not in st.session_state:
            st.session_state["surveillance_data.csv"] = pd.DataFrame(columns=data.keys())
        df = st.session_state["surveillance_data.csv"]
        df = pd.concat([df, pd.DataFrame([data])], ignore_index=True)
        st.session_state["surveillance_data.csv"] = df
...
if "surveillance_data.csv" in st.session_state:
    st.subheader("Historique des données enregistrées")
    st.dataframe(st.session_state["surveillance_data.csv"])
